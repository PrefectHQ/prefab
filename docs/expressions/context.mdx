...
The `visible_when` prop uses expressions to control whether a component renders. The expression must evaluate to a truthy or falsy value. This is one of the most practical uses of expression context — combining state from different sources to determine what the user should see:

<ComponentPreview auto json={`{"view":{"cssClass":"gap-3","type":"Column","children":[{"type":"Text","content":"Inventory: {{ inventory }}"},{"type":"Slider","min":0.0,"max":30.0,"step":1.0,"name":"inventory","disabled":false,"onChange":{"action":"setState","key":"inventory","value":"{{ $event }}"}},{"visibleWhen":"inventory == 0","type":"Alert","variant":"destructive","children":[{"type":"AlertDescription","content":"Out of stock!"}]},{"visibleWhen":"inventory > 0 && inventory < 10","type":"Alert","variant":"warning","children":[{"type":"AlertDescription","content":"Low stock warning"}]}]},"inventory":15}`}>
<CodeGroup>
```python Python icon="python"
...
with Column(gap=3):
    ...
    Slider(
        ...
        max=30,
        step=1,
        on_change=SetState("inventory"),
    )
    with Alert(variant="destructive", visible_when="inventory == 0"):
        AlertDescription("Out of stock!")
    with Alert(
        variant="warning",
        visible_when="inventory > 0 && inventory < 10",
    ):
...
```json Protocol icon="brackets-curly"
{
  "view": {
    ...
    "children": [
      ...
      {
        "visibleWhen": "inventory > 0 && inventory < 10",
        "type": "Alert",
        "variant": "warning",
        "children": [
          {
