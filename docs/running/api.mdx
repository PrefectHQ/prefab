...
item_name = Input(name="item_name", placeholder="Name")
item_category = Input(name="item_category", placeholder="Category")

with Form(
    on_submit=Fetch.post(
        "/api/items",
        body={"name": item_name.rx, "category": item_category.rx},
...
name_input = Input(name="new_name", placeholder="Name")
