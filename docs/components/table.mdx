...
A table is composed from nested sub-components that mirror HTML table structure: a `Table` wraps a `TableHeader` and a `TableBody`, each holding `TableRow`s. Inside a row, use `TableHead` for header labels and `TableCell` for body data, and add an optional `TableCaption` to title the whole table. Build the structure with nested `with` blocks so the indentation matches the table's shape.
