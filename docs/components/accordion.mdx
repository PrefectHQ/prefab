...
<ParamField body="default_open_items" type="number | string | Action[] | null">
  Initially expanded item(s). Pass an int for index-based selection, or a str to match by value/title.
</ParamField>

<ParamField body="defaultValues" type="Action[] | null">
  Wire format for default_open_items (always an array).
</ParamField>
