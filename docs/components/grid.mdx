...
Set `min_column_width` instead of `columns` to let CSS Grid automatically fit as many columns as possible, each at least the given width. The grid reflows based on the container's actual width — try resizing your browser to see it in action.

<ComponentPreview auto json={`{"view":{"cssClass":"gap-4 grid-cols-[repeat(auto-fill,minmax(10rem,1fr))]","type":"Grid","children":[{"cssClass":"bg-emerald-500 rounded-md h-10","type":"Div"},{"cssClass":"bg-emerald-500 rounded-md h-10","type":"Div"},{"cssClass":"bg-emerald-500 rounded-md h-10","type":"Div"},{"cssClass":"bg-emerald-500 rounded-md h-10","type":"Div"},{"cssClass":"bg-emerald-500 rounded-md h-10","type":"Div"},{"cssClass":"bg-emerald-500 rounded-md h-10","type":"Div"}]}}`}>
<CodeGroup>
```python Python icon="python"
from prefab_ui.components import Div, Grid
...
{
  "view": {
    "cssClass": "gap-4 grid-cols-[repeat(auto-fill,minmax(10rem,1fr))]",
    "type": "Grid",
    "children": [
      {
        "cssClass": "bg-emerald-500 rounded-md h-10",
...
Pass a dictionary of breakpoints to column counts when you need precise control over the layout at specific viewport widths. The `default` key sets the base, and Tailwind breakpoints (`sm`, `md`, `lg`, `xl`, `2xl`) override it progressively.

```python
from prefab_ui.components import Grid, Responsive

# Dictionary shorthand
Grid(columns={"default": 1, "md": 2, "lg": 3})

# Responsive object — equivalent, but works on any layout property
Grid(columns=Responsive(default=1, md=2, lg=3))
```

<Note>
Breakpoints respond to the **viewport** width, not the container. If your UI renders inside a narrow panel, `min_column_width` (auto-fill) is often a better fit since it reflows based on available space.
</Note>
