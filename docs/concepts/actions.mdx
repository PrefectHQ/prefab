...
Actions update state, call servers, and give UI feedback in response to events. A button click, a form submission, or a slider change can each trigger an action.
...
Actions can also run when a component mounts, on a timer, or through a keyboard shortcut. In an MCP host, [app-provided tools](/mcp/overview#app-provided-tools) let an agent invoke an interaction against the same live state. Each trigger uses the same actions and response callbacks.
...
An action's destination determines what it can do and how its response is handled.

**Client actions** run entirely in the browser. `SetState`, `ToggleState`, `AppendState`, `ShowToast`, `OpenLink`, and `SetInterval` manage UI state and give immediate feedback without a server request.
...
**Host actions** communicate with the [MCP host](/mcp/overview) displaying the app. They can send a message to the conversation, share context with the model, or request a display mode. Their behavior depends on the host's support.

An interaction often combines a server request with local state updates. A "Save" button sets a loading flag, calls the server, then shows success or failure feedback through response actions.
...
|----------------|---------|
| [`CallTool`](/actions/call-tool) | Call an MCP tool; result available as `$result` in `on_success` |
| `Fetch` | Make an HTTP request; result available as `$result` in `on_success` |

| Host actions | Purpose |
|--------------|---------|
| [`SendMessage`](/actions/send-message) | Send a user message to the conversation |
| [`UpdateContext`](/actions/update-context) | Share context for future model turns |
| [`RequestDisplayMode`](/actions/request-display-mode) | Request inline, fullscreen, or picture-in-picture display |
