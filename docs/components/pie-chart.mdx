---
title: PieChart
description: Visualize proportions with slices of a circle.
icon: chart-pie
---

import { ComponentPreview } from '/snippets/component-preview.mdx'

PieChart shows how parts relate to a whole. Unlike the other chart types, it doesn't use series — instead you specify which data field holds the numeric values (`data_key`) and which holds the labels (`name_key`). Set `inner_radius` to turn it into a donut chart.

## Basic Usage

<ComponentPreview auto json={`{"view":{"type":"PieChart","data":[{"browser":"Chrome","visitors":275},{"browser":"Safari","visitors":200},{"browser":"Firefox","visitors":187},{"browser":"Edge","visitors":173},{"browser":"Other","visitors":90}],"dataKey":"visitors","nameKey":"browser","height":300,"innerRadius":0,"showLabel":false,"paddingAngle":0,"showLegend":true,"showTooltip":true}}`}>
<CodeGroup>
```python Python icon="python"
from prefab_ui.components import PieChart

data = [
    {"browser": "Chrome", "visitors": 275},
    {"browser": "Safari", "visitors": 200},
    {"browser": "Firefox", "visitors": 187},
    {"browser": "Edge", "visitors": 173},
    {"browser": "Other", "visitors": 90},
]

PieChart(
    data=data,
    data_key="visitors",
    name_key="browser",
    show_legend=True,
)
```
```json Protocol icon="brackets-curly"
{
  "view": {
    "type": "PieChart",
    "data": [
      {
        "browser": "Chrome",
        "visitors": 275
      },
      {
        "browser": "Safari",
        "visitors": 200
      },
      {
        "browser": "Firefox",
        "visitors": 187
      },
      {
        "browser": "Edge",
        "visitors": 173
      },
      {
        "browser": "Other",
        "visitors": 90
      }
    ],
    "dataKey": "visitors",
    "nameKey": "browser",
    "height": 300,
    "innerRadius": 0,
    "showLabel": false,
    "paddingAngle": 0,
    "showLegend": true,
    "showTooltip": true
  }
}
```
</CodeGroup>
</ComponentPreview>

## Donut Chart

Set `inner_radius` to a value greater than 0 to hollow out the center. This creates a donut chart, which can feel cleaner and leaves room for a central label or statistic.

<ComponentPreview auto json={`{"view":{"type":"PieChart","data":[{"browser":"Chrome","visitors":275},{"browser":"Safari","visitors":200},{"browser":"Firefox","visitors":187},{"browser":"Edge","visitors":173},{"browser":"Other","visitors":90}],"dataKey":"visitors","nameKey":"browser","height":300,"innerRadius":60,"showLabel":false,"paddingAngle":0,"showLegend":true,"showTooltip":true}}`}>
<CodeGroup>
```python Python icon="python"
PieChart(
    data=[
        {"browser": "Chrome", "visitors": 275},
        {"browser": "Safari", "visitors": 200},
        {"browser": "Firefox", "visitors": 187},
        {"browser": "Edge", "visitors": 173},
        {"browser": "Other", "visitors": 90},
    ],
    data_key="visitors",
    name_key="browser",
    inner_radius=60,
    show_legend=True,
)
```
```json Protocol icon="brackets-curly"
{
  "view": {
    "type": "PieChart",
    "data": [
      {
        "browser": "Chrome",
        "visitors": 275
      },
      {
        "browser": "Safari",
        "visitors": 200
      },
      {
        "browser": "Firefox",
        "visitors": 187
      },
      {
        "browser": "Edge",
        "visitors": 173
      },
      {
        "browser": "Other",
        "visitors": 90
      }
    ],
    "dataKey": "visitors",
    "nameKey": "browser",
    "height": 300,
    "innerRadius": 60,
    "showLabel": false,
    "paddingAngle": 0,
    "showLegend": true,
    "showTooltip": true
  }
}
```
</CodeGroup>
</ComponentPreview>

## Labels

Set `show_label=True` to display a label on each slice, making values readable without tooltips.

<ComponentPreview auto json={`{"view":{"type":"PieChart","data":[{"browser":"Chrome","visitors":275},{"browser":"Safari","visitors":200},{"browser":"Firefox","visitors":187},{"browser":"Edge","visitors":173},{"browser":"Other","visitors":90}],"dataKey":"visitors","nameKey":"browser","height":300,"innerRadius":0,"showLabel":true,"paddingAngle":0,"showLegend":true,"showTooltip":true}}`}>
<CodeGroup>
```python Python {5} icon="python"
PieChart(
    data=data,
    data_key="visitors",
    name_key="browser",
    show_label=True,
    show_legend=True,
)
```
```json Protocol icon="brackets-curly"
{
  "view": {
    "type": "PieChart",
    "data": [
      {
        "browser": "Chrome",
        "visitors": 275
      },
      {
        "browser": "Safari",
        "visitors": 200
      },
      {
        "browser": "Firefox",
        "visitors": 187
      },
      {
        "browser": "Edge",
        "visitors": 173
      },
      {
        "browser": "Other",
        "visitors": 90
      }
    ],
    "dataKey": "visitors",
    "nameKey": "browser",
    "height": 300,
    "innerRadius": 0,
    "showLabel": true,
    "paddingAngle": 0,
    "showLegend": true,
    "showTooltip": true
  }
}
```
</CodeGroup>
</ComponentPreview>

## Spaced Slices

Set `padding_angle` to add space between slices, visually separating each segment.

<ComponentPreview auto json={`{"view":{"type":"PieChart","data":[{"browser":"Chrome","visitors":275},{"browser":"Safari","visitors":200},{"browser":"Firefox","visitors":187},{"browser":"Edge","visitors":173},{"browser":"Other","visitors":90}],"dataKey":"visitors","nameKey":"browser","height":300,"innerRadius":60,"showLabel":false,"paddingAngle":5,"showLegend":true,"showTooltip":true}}`}>
<CodeGroup>
```python Python {6} icon="python"
PieChart(
    data=data,
    data_key="visitors",
    name_key="browser",
    inner_radius=60,
    padding_angle=5,
    show_legend=True,
)
```
```json Protocol icon="brackets-curly"
{
  "view": {
    "type": "PieChart",
    "data": [
      {
        "browser": "Chrome",
        "visitors": 275
      },
      {
        "browser": "Safari",
        "visitors": 200
      },
      {
        "browser": "Firefox",
        "visitors": 187
      },
      {
        "browser": "Edge",
        "visitors": 173
      },
      {
        "browser": "Other",
        "visitors": 90
      }
    ],
    "dataKey": "visitors",
    "nameKey": "browser",
    "height": 300,
    "innerRadius": 60,
    "showLabel": false,
    "paddingAngle": 5,
    "showLegend": true,
    "showTooltip": true
  }
}
```
</CodeGroup>
</ComponentPreview>

## API Reference

<Card icon="code" title="PieChart Parameters">
<ParamField body="data" type="list[dict] | str" required>
  Data as a list of dicts, or a `{{ field }}` interpolation reference.
</ParamField>

<ParamField body="data_key" type="str" required>
  The data field containing numeric values to plot as slice sizes.
</ParamField>

<ParamField body="name_key" type="str" required>
  The data field containing labels for each slice.
</ParamField>

<ParamField body="height" type="int" default="300">
  Chart height in pixels.
</ParamField>

<ParamField body="inner_radius" type="int" default="0">
  Inner radius in pixels. Set greater than 0 for a donut chart.
</ParamField>

<ParamField body="show_label" type="bool" default="False">
  Show a label on each slice.
</ParamField>

<ParamField body="padding_angle" type="int" default="0">
  Angle in degrees between slices.
</ParamField>

<ParamField body="show_legend" type="bool" default="False">
  Show a legend below the chart.
</ParamField>

<ParamField body="show_tooltip" type="bool" default="True">
  Show tooltips on hover.
</ParamField>

<ParamField body="css_class" type="str | None" default="None">
  Additional Tailwind CSS classes.
</ParamField>
</Card>

## Protocol Reference

<Card icon="braces" title="PieChart Protocol">
<ParamField body="cssClass" type="string | null">
  CSS/Tailwind classes for styling
</ParamField>

<ParamField body="type" type="PieChart" default="PieChart">
  Component discriminator.
</ParamField>

<ParamField body="data" type="Action[] | string" required>
  Row data or `{{ interpolation }}` reference
</ParamField>

<ParamField body="dataKey" type="string" required>
  Numeric value field
</ParamField>

<ParamField body="nameKey" type="string" required>
  Label field
</ParamField>

<ParamField body="height" type="number" default="300">
  Chart height in pixels
</ParamField>

<ParamField body="innerRadius" type="number" default="0">
  Inner radius (>0 for donut)
</ParamField>

<ParamField body="showLabel" type="boolean" default="false">
  Show labels on slices
</ParamField>

<ParamField body="paddingAngle" type="number" default="0">
  Gap between slices in degrees
</ParamField>

<ParamField body="showLegend" type="boolean" default="false">
  Show legend
</ParamField>

<ParamField body="showTooltip" type="boolean" default="true">
  Show tooltip on hover
</ParamField>

</Card>
