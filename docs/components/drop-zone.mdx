...
<Info>
  All file reading happens in the browser. Files are never sent to a server unless you explicitly wire up an action (like `ToolCall`) to do so.
</Info>
...
<ComponentPreview json={{"view":{"type":"DropZone","multiple":false,"disabled":false}}}>
<CodeGroup>
...
<ComponentPreview json={{"view":{"type":"DropZone","icon":"image","label":"Drop images here","description":"PNG, JPG, or GIF up to 10MB","multiple":false,"disabled":false}}}>
<CodeGroup>
```python Python icon="python"
from prefab_ui.components import DropZone
...
<ComponentPreview json={{"view":{"type":"DropZone","label":"Upload documents","description":"PDF, DOCX, or TXT","accept":".pdf,.docx,.txt","multiple":false,"disabled":false}}}>
<CodeGroup>
```python Python icon="python"
from prefab_ui.components import DropZone
...
<ComponentPreview json={{"view":{"type":"DropZone","label":"Upload files","description":"Select as many as you like","multiple":true,"disabled":false}}}>
<CodeGroup>
```python Python icon="python"
from prefab_ui.components import DropZone
...
Files stay in the browser until you do something with them. The `on_change` parameter fires actions when files are selected, with `$event` containing the file data — so the typical pattern is to pair it with a `ToolCall` that sends the base64 payload to your MCP server for processing.

```python DropZone with ToolCall
from prefab_ui.components import DropZone
...
DropZone(
    ...
    on_change=[
        ...
        ToolCall("upload_file", arguments={"file": "{{ $event }}"}),
...
<ComponentPreview json={{"view":{"type":"DropZone","label":"Uploads disabled","multiple":false,"disabled":true}}}>
<CodeGroup>
```python Python icon="python"
from prefab_ui.components import DropZone
