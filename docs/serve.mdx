...
This opens your browser to `http://127.0.0.1:5175` with the rendered UI. All client-side behavior works — state binding, `SetState`/`ToggleState` actions, `ForEach` loops, conditional rendering, form submissions. The only thing that won't work is `ToolCall`, since there's no MCP server to call back to.
