...
<ComponentPreview json={{"view":{"cssClass":"gap-3","type":"Column","children":[{"name":"slider_21","type":"Slider","min":0.0,"max":100.0,"value":50.0,"disabled":false},{"content":"Volume: {{ slider_21 }}%","type":"Text"},{"type":"Progress","value":"{{ slider_21 }}","variant":"default"}]},"state":{"slider_21":50.0}}} playground="">
<CodeGroup>
```python Python icon="python"
...
```json Protocol icon="brackets-curly"
{
  "view": {
    ...
    "children": [
      ...
      {"content": "Volume: {{ slider_21 }}%", "type": "Text"},
      {"type": "Progress", "value": "{{ slider_21 }}", "variant": "default"}
    ]
  },
  "state": {"slider_21": 50.0}
}
```
</CodeGroup>
</ComponentPreview>
...
revenue.compact()          # {{ revenue | compact }}        → 2.8M
revenue.compact(0)         # {{ revenue | compact:0 }}      → 3M
revenue.round(2)           # {{ revenue | round:2 }}        → 2847500.00
revenue.abs()              # {{ revenue | abs }}            → absolute value
```

`.currency()` and `.number()` add locale-aware grouping separators. `.compact()` abbreviates large numbers using compact notation (`1.8M`, `470K`); small values pass through unchanged. `.round()` rounds without adding separators, which is useful for computations that feed into other expressions.
