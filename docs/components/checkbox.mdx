...
<ComponentPreview auto json={`{"$protocol":"prefab","view":{"type":"Checkbox","label":"Accept terms and conditions","checked":false,"disabled":false,"required":false}}`}>
<CodeGroup>
...
<ComponentPreview auto json={`{"$protocol":"prefab","view":{"type":"Checkbox","label":"Subscribe to newsletter","checked":true,"disabled":false,"required":false}}`}>
<CodeGroup>
```python Python icon="python"
from prefab_ui.components import Checkbox
