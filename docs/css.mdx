---
title: CSS Helpers
description: Compose Tailwind classes without repeating variant prefixes.
---
...
Layout props like `columns` and `gap` accept a separate `Responsive` helper that translates values into the right Tailwind classes — see [Responsive Layouts](/patterns/responsive-layouts) for that pattern.
