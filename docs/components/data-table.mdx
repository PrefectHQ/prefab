---
title: DataTable
description: High-level data table with sorting, filtering, and pagination.
icon: table-cells
---

import { ComponentPreview } from '/snippets/component-preview.mdx'

DataTable takes flat `columns` and `rows` and renders a full-featured table with optional sorting, search filtering, and pagination. Use it when you have structured data and want interactive table features without building the markup yourself.

## Basic Usage

<ComponentPreview json={{"view":{"type":"DataTable","columns":[{"key":"name","header":"Name","sortable":true},{"key":"email","header":"Email","sortable":false},{"key":"role","header":"Role","sortable":true}],"rows":[{"name":"Alice Johnson","email":"alice@example.com","role":"Admin"},{"name":"Bob Smith","email":"bob@example.com","role":"Editor"},{"name":"Carol White","email":"carol@example.com","role":"Viewer"}],"filter":true,"paginated":false,"pageSize":10}}} playground="H4sIADZOuGkC_4WQzUoDMRDH73mKkFMLiw8gLKi1lx482FIPIjLJTkkwySxJliriu5vslnUrW51L5j8fv5nMIZDjbcADyNfOXClyLXn0KXKTvZD4PSTYgbRY_bgrsp3zjI2BBePZVB-O9XOviv3qWLzhRy08OBQV1wgNhlo8DDLmYaWy3oUOl9XfCHRg7ISx7vV_XYHsdPDjIOcGvwxPoOP0N5_D6tdc3FqjkG9I-0g-M04L5QyUzA2-g2stlnOWbD-4tDXOePFVzRDvSPKtM0mf0STJS6x1YxKFedgKchV_0ibhGU6V-CXg3uARR-DpAhEhKD2ep2JL9g1S0usAMgIAAA">
<CodeGroup>
```python Python icon="python"
from prefab_ui.components import DataTable, DataTableColumn

DataTable(
    columns=[
        DataTableColumn(key="name", header="Name", sortable=True),
        DataTableColumn(key="email", header="Email"),
        DataTableColumn(key="role", header="Role", sortable=True),
    ],
    rows=[
        {"name": "Alice Johnson", "email": "alice@example.com", "role": "Admin"},
        {"name": "Bob Smith", "email": "bob@example.com", "role": "Editor"},
        {"name": "Carol White", "email": "carol@example.com", "role": "Viewer"},
    ],
    filter=True,
)
```
```json Protocol icon="brackets-curly"
{
  "view": {
    "type": "DataTable",
    "columns": [
      {"key": "name", "header": "Name", "sortable": true},
      {"key": "email", "header": "Email", "sortable": false},
      {"key": "role", "header": "Role", "sortable": true}
    ],
    "rows": [
      {"name": "Alice Johnson", "email": "alice@example.com", "role": "Admin"},
      {"name": "Bob Smith", "email": "bob@example.com", "role": "Editor"},
      {"name": "Carol White", "email": "carol@example.com", "role": "Viewer"}
    ],
    "filter": true,
    "paginated": false,
    "pageSize": 10
  }
}
```
</CodeGroup>
</ComponentPreview>

## Pagination

Enable pagination to handle large datasets. The table shows page controls and displays `page_size` rows at a time.

<ComponentPreview json={{"view":{"type":"DataTable","columns":[{"key":"id","header":"ID","sortable":false},{"key":"task","header":"Task","sortable":true},{"key":"status","header":"Status","sortable":false}],"rows":[{"id":"T-001","task":"Design system","status":"Complete"},{"id":"T-002","task":"API integration","status":"In Progress"},{"id":"T-003","task":"Testing","status":"Pending"},{"id":"T-004","task":"Documentation","status":"In Progress"},{"id":"T-005","task":"Deployment","status":"Pending"}],"filter":false,"paginated":true,"pageSize":3}}} playground="H4sIADZOuGkC_52RwWrDMAyG73kK41MLXmnX7TLIYTSX3ALLbYziJmpmmtjBUhjZ2LvPTkLnjLWD6WLrl_XxWzpa07DWwlEe9p1aFaZpjQZNyJS7WWKJJJnLQw3i-7ozddfoKDoLi4i5KAYZ4-ch8_GjY3GCPuaq5IK9gizBxjxN-FJcf08ST0FHPqbozPmXcW47-AuBJKnDAPI0ClPfy3hY8xaa__BOHxjPb9brjesdjTghAVSVZtgjQeMLE96Vdm5-NRDwT_Er5zbkPGYpU5qgspKU0XNSqllmTWUB8RJsG8JyQFK6mkMy0KUXLwDuZr8yRde4zf_Py_18QG1teg-7amcaeysrpSVBOazyrMEe1TvEWxEtoy8SxEsfpgIAAA">
<CodeGroup>
```python Python icon="python"
from prefab_ui.components import DataTable, DataTableColumn

DataTable(
    columns=[
        DataTableColumn(key="id", header="ID"),
        DataTableColumn(key="task", header="Task", sortable=True),
        DataTableColumn(key="status", header="Status"),
    ],
    rows=[
        {"id": "T-001", "task": "Design system", "status": "Complete"},
        {"id": "T-002", "task": "API integration", "status": "In Progress"},
        {"id": "T-003", "task": "Testing", "status": "Pending"},
        {"id": "T-004", "task": "Documentation", "status": "In Progress"},
        {"id": "T-005", "task": "Deployment", "status": "Pending"},
    ],
    paginated=True,
    page_size=3,
)
```
```json Protocol icon="brackets-curly"
{
  "view": {
    "type": "DataTable",
    "columns": [
      {"key": "id", "header": "ID", "sortable": false},
      {"key": "task", "header": "Task", "sortable": true},
      {"key": "status", "header": "Status", "sortable": false}
    ],
    "rows": [
      {"id": "T-001", "task": "Design system", "status": "Complete"},
      {"id": "T-002", "task": "API integration", "status": "In Progress"},
      {"id": "T-003", "task": "Testing", "status": "Pending"},
      {"id": "T-004", "task": "Documentation", "status": "In Progress"},
      {"id": "T-005", "task": "Deployment", "status": "Pending"}
    ],
    "filter": false,
    "paginated": true,
    "pageSize": 3
  }
}
```
</CodeGroup>
</ComponentPreview>

## Component Cells

Cell values can be Prefab components — not just strings. Put a Badge, Sparkline, Progress bar, or any other component directly in the row data and it renders inline.

<ComponentPreview json={{"view":{"type":"DataTable","columns":[{"key":"metric","header":"Metric","sortable":true},{"key":"trend","header":"Trend","sortable":false},{"key":"status","header":"Status","sortable":false}],"rows":[{"metric":"Revenue","trend":{"cssClass":"w-24","type":"Sparkline","data":[45,52,49,60,55,72,68,75],"variant":"success","fill":true,"curve":"linear","strokeWidth":1.5},"status":{"type":"Badge","label":"Growing","variant":"success"}},{"metric":"Churn","trend":{"cssClass":"w-24","type":"Sparkline","data":[12,15,18,14,20,22,19,25],"variant":"destructive","fill":false,"curve":"linear","strokeWidth":1.5},"status":{"type":"Badge","label":"At risk","variant":"destructive"}},{"metric":"Users","trend":{"cssClass":"w-24","type":"Sparkline","data":[250,248,252,249,253,251,250,252],"variant":"info","fill":true,"curve":"linear","strokeWidth":1.5},"status":{"type":"Badge","label":"Stable","variant":"secondary"}}],"filter":false,"paginated":false,"pageSize":10}}} playground="H4sIADZOuGkC_62T30_CMBDH3_dXNHuCpBo3N0WSPSgmPpkYwSdiSOlu0DA6cu0gxvi_e91ABijGH_fQ3bW9b-8-7TIs5myBkInxqFSnspgvCg3aGqbIQ8tuhRUDMc6Bb91ekZdzzdmNSCc0_4DFBMEYL_tK7FROBW41-wuBs1xp8LwPzZbHyGSlbJJhFTnbO7Q1g5fEn4NFJX3OpiBSwMS_30wY0ne7kwGW0ObHZSyCThsqgyr-LstYYUvTSOvXE-u85_qDxarZxuuH52zTQJf5j7AEXYLPdzfUpXW3qFo7685SKi4ZRjFncchZdMXZxRn5FF9SfNGhb_zMD9KWApXQlvoopaRbo0YylecVMM6kMSOZC2MSf3USRntltfeqXKPo1i-h5d9R10pPSPLglEbqG_8eS29aov41lIAABAQiIAhBxFlIYEI3R5DCo1BSMBZLadWSruSPNK4tQ2VmTRpN-R8SeTKA5tdEwtghiDqu_9B5FYlzNwRuOKsWjpFROiv-8630q_9056mALHQq8OUTNFRZ23sHQV8w-a0EAAA">
<CodeGroup>
```python Python expandable icon="python"
from prefab_ui.components import DataTable, DataTableColumn, Badge, Progress
from prefab_ui.components.charts import Sparkline

DataTable(
    columns=[
        DataTableColumn(key="metric", header="Metric", sortable=True),
        DataTableColumn(key="trend", header="Trend"),
        DataTableColumn(key="status", header="Status"),
    ],
    rows=[
        {
            "metric": "Revenue",
            "trend": Sparkline(
                data=[45, 52, 49, 60, 55, 72, 68, 75],
                variant="success", fill=True, css_class="w-24",
            ),
            "status": Badge("Growing", variant="success"),
        },
        {
            "metric": "Churn",
            "trend": Sparkline(
                data=[12, 15, 18, 14, 20, 22, 19, 25],
                variant="destructive", css_class="w-24",
            ),
            "status": Badge("At risk", variant="destructive"),
        },
        {
            "metric": "Users",
            "trend": Sparkline(
                data=[250, 248, 252, 249, 253, 251, 250, 252],
                variant="info", fill=True, css_class="w-24",
            ),
            "status": Badge("Stable", variant="secondary"),
        },
    ],
)
```
```json Protocol icon="brackets-curly"
{
  "view": {
    "type": "DataTable",
    "columns": [
      {"key": "metric", "header": "Metric", "sortable": true},
      {"key": "trend", "header": "Trend", "sortable": false},
      {"key": "status", "header": "Status", "sortable": false}
    ],
    "rows": [
      {
        "metric": "Revenue",
        "trend": {
          "cssClass": "w-24",
          "type": "Sparkline",
          "data": [45, 52, 49, 60, 55, 72, 68, 75],
          "variant": "success",
          "fill": true,
          "curve": "linear",
          "strokeWidth": 1.5
        },
        "status": {"type": "Badge", "label": "Growing", "variant": "success"}
      },
      {
        "metric": "Churn",
        "trend": {
          "cssClass": "w-24",
          "type": "Sparkline",
          "data": [12, 15, 18, 14, 20, 22, 19, 25],
          "variant": "destructive",
          "fill": false,
          "curve": "linear",
          "strokeWidth": 1.5
        },
        "status": {"type": "Badge", "label": "At risk", "variant": "destructive"}
      },
      {
        "metric": "Users",
        "trend": {
          "cssClass": "w-24",
          "type": "Sparkline",
          "data": [250, 248, 252, 249, 253, 251, 250, 252],
          "variant": "info",
          "fill": true,
          "curve": "linear",
          "strokeWidth": 1.5
        },
        "status": {"type": "Badge", "label": "Stable", "variant": "secondary"}
      }
    ],
    "filter": false,
    "paginated": false,
    "pageSize": 10
  }
}
```
</CodeGroup>
</ComponentPreview>

## Cell Formatting

The `format` field on `DataTableColumn` formats numeric and date values without requiring a component cell. Use `'type'` or `'type:arg'` syntax to control the output.

<ComponentPreview json={{"view":{"type":"DataTable","columns":[{"key":"product","header":"Product","sortable":false},{"key":"revenue","header":"Revenue","sortable":true,"format":"currency"},{"key":"growth","header":"Growth","sortable":true,"format":"percent:1"},{"key":"units","header":"Units","sortable":true,"format":"number"},{"key":"launched","header":"Launched","sortable":false,"format":"date:medium"}],"rows":[{"product":"Widget Pro","revenue":128450,"growth":0.142,"units":3210,"launched":"2023-03-15"},{"product":"Gadget Plus","revenue":84200,"growth":0.087,"units":1980,"launched":"2022-11-01"},{"product":"Doohickey","revenue":52100,"growth":-0.031,"units":870,"launched":"2024-01-20"}],"searchable":false,"paginated":false,"pageSize":10}}}>
<CodeGroup>
```python Python icon="python"
from prefab_ui.components import DataTable, DataTableColumn

DataTable(
    columns=[
        DataTableColumn(key="product", header="Product"),
        DataTableColumn(key="revenue", header="Revenue", sortable=True, format="currency"),
        DataTableColumn(key="growth", header="Growth", sortable=True, format="percent:1"),
        DataTableColumn(key="units", header="Units", sortable=True, format="number"),
        DataTableColumn(key="launched", header="Launched", format="date:medium"),
    ],
    rows=[
        {"product": "Widget Pro", "revenue": 128450, "growth": 0.142, "units": 3210, "launched": "2023-03-15"},
        {"product": "Gadget Plus", "revenue": 84200, "growth": 0.087, "units": 1980, "launched": "2022-11-01"},
        {"product": "Doohickey", "revenue": 52100, "growth": -0.031, "units": 870, "launched": "2024-01-20"},
    ],
)
```
```json Protocol icon="brackets-curly"
{
  "view": {
    "type": "DataTable",
    "columns": [
      {"key": "product", "header": "Product"},
      {"key": "revenue", "header": "Revenue", "sortable": true, "format": "currency"},
      {"key": "growth", "header": "Growth", "sortable": true, "format": "percent:1"},
      {"key": "units", "header": "Units", "sortable": true, "format": "number"},
      {"key": "launched", "header": "Launched", "format": "date:medium"}
    ],
    "rows": [
      {"product": "Widget Pro", "revenue": 128450, "growth": 0.142, "units": 3210, "launched": "2023-03-15"},
      {"product": "Gadget Plus", "revenue": 84200, "growth": 0.087, "units": 1980, "launched": "2022-11-01"},
      {"product": "Doohickey", "revenue": 52100, "growth": -0.031, "units": 870, "launched": "2024-01-20"}
    ],
    "searchable": false,
    "paginated": false,
    "pageSize": 10
  }
}
```
</CodeGroup>
</ComponentPreview>

Supported format strings:

| Format | Example output | Notes |
|--------|---------------|-------|
| `number` | `1,234,567` | Locale thousands separator |
| `number:2` | `1,234.50` | Fixed decimal places |
| `currency` | `$1,234.50` | USD by default |
| `currency:EUR` | `€1,234.50` | Any ISO 4217 code |
| `percent` | `14%` | Value should be `0.14`, not `14` |
| `percent:1` | `14.2%` | Fixed decimal places |
| `date` | `Mar 15, 2023` | Medium style by default |
| `date:short` | `3/15/23` | short / medium / long / full |
| `date:long` | `March 15, 2023` | |

## Column Widths

Set explicit widths on columns to control the table layout. Without widths, columns auto-size to their content — which is fine for text, but numbers and compact values like IDs benefit from fixed widths.

<ComponentPreview>
```python Python icon="python"
from prefab_ui.components import DataTable, DataTableColumn

DataTable(
    columns=[
        DataTableColumn(key="id", header="ID", width="80px"),
        DataTableColumn(key="name", header="Name"),
        DataTableColumn(key="amount", header="Amount", width="120px"),
    ],
    rows=[
        {"id": "001", "name": "Alice Johnson", "amount": "$1,200.00"},
        {"id": "002", "name": "Bob Smith", "amount": "$850.50"},
        {"id": "003", "name": "Carol White", "amount": "$2,100.00"},
    ],
)
```
</ComponentPreview>

## Column Alignment

Right-align numbers and currency so decimal points line up. Center short values like status codes or icons. The `align` prop is a convenience that compiles to `cell_class` Python-side — the protocol only sees the CSS class.

<ComponentPreview>
```python Python icon="python"
from prefab_ui.components import DataTable, DataTableColumn

DataTable(
    columns=[
        DataTableColumn(key="name", header="Name"),
        DataTableColumn(key="region", header="Region", align="center"),
        DataTableColumn(key="revenue", header="Revenue", align="right"),
        DataTableColumn(key="growth", header="Growth", align="right"),
    ],
    rows=[
        {"name": "Acme Corp", "region": "US", "revenue": "$4.2M", "growth": "+12%"},
        {"name": "Globex", "region": "EU", "revenue": "$2.8M", "growth": "-3%"},
        {"name": "Initech", "region": "US", "revenue": "$1.5M", "growth": "+28%"},
    ],
)
```
</ComponentPreview>

## Column Styling

Use `header_class` and `cell_class` for arbitrary Tailwind on individual columns — font weights, colors, truncation, monospace for numbers. These are the escape hatches when `align` and `width` aren't enough.

<ComponentPreview>
```python Python icon="python"
from prefab_ui.components import DataTable, DataTableColumn

DataTable(
    columns=[
        DataTableColumn(
            key="name", header="Name",
            cell_class="font-semibold",
        ),
        DataTableColumn(key="email", header="Email", cell_class="text-muted-foreground"),
        DataTableColumn(
            key="amount", header="Amount",
            align="right",
            cell_class="font-mono",
        ),
    ],
    rows=[
        {"name": "Alice Johnson", "email": "alice@example.com", "amount": "$1,200.00"},
        {"name": "Bob Smith", "email": "bob@example.com", "amount": "$850.50"},
        {"name": "Carol White", "email": "carol@example.com", "amount": "$2,100.00"},
    ],
)
```
</ComponentPreview>

## Column Pattern

`DataTableColumn` defines everything about a column in one place — what data to show, how to display it, and how it behaves. Here's the full set of options:

```python
DataTableColumn(
    key="revenue",          # which field from row data
    header="Revenue ($)",   # display header
    sortable=True,          # click-to-sort
    width="150px",          # fixed width
    align="right",          # text alignment (→ cell_class)
    cell_class="font-mono", # arbitrary Tailwind on cells
    header_class="text-right font-bold",  # arbitrary Tailwind on header
)
```

## API Reference

<Card icon="code" title="DataTable Parameters">
<ParamField body="columns" type="list[DataTableColumn]" required>
  Column definitions. Each column specifies a `key`, `header`, and optional `sortable` flag.
</ParamField>

<ParamField body="rows" type="list[dict] | str" default="[]">
  Row data as a list of dicts, or a `{{ field }}` interpolation reference.
</ParamField>

<ParamField body="filter" type="bool" default="False">
  Show a search/filter input above the table.
</ParamField>

<ParamField body="paginated" type="bool" default="False">
  Show pagination controls below the table.
</ParamField>

<ParamField body="page_size" type="int" default="10">
  Number of rows per page when `paginated=True`.
</ParamField>

<ParamField body="css_class" type="str | None" default="None">
  Additional Tailwind CSS classes.
</ParamField>
</Card>

<Card icon="code" title="DataTableColumn Parameters">
<ParamField body="key" type="str" required>
  Data key — matches keys in the row dicts.
</ParamField>

<ParamField body="header" type="str" required>
  Column header display text.
</ParamField>

<ParamField body="sortable" type="bool" default="False">
  Enable click-to-sort on this column.
</ParamField>

<ParamField body="format" type="str | None" default="None">
  Format string for cell values. Accepts `number`, `number:N`, `currency`, `currency:CODE`,
  `percent`, `percent:N`, `date`, `date:short|medium|long|full`. Ignored when the cell value is a component.
</ParamField>

<ParamField body="width" type="str | None" default="None">
  Column width CSS value (e.g. `"200px"`, `"30%"`).
</ParamField>

<ParamField body="min_width" type="str | None" default="None">
  Minimum column width CSS value.
</ParamField>

<ParamField body="max_width" type="str | None" default="None">
  Maximum column width CSS value.
</ParamField>

<ParamField body="align" type='"left" | "center" | "right" | None' default="None">
  Cell text alignment. Resolves Python-side to `cell_class` — does not appear in the protocol.
</ParamField>

<ParamField body="header_class" type="str | None" default="None">
  Tailwind classes applied to the header cell for this column.
</ParamField>

<ParamField body="cell_class" type="str | None" default="None">
  Tailwind classes applied to every data cell in this column.
</ParamField>
</Card>

## Protocol Reference

```json DataTable
{
  "type": "DataTable",
  "columns": "[DataTableColumn] (required)",
  "rows?": "Action[] | string",
  "filter?": false,
  "paginated?": false,
  "pageSize?": 10,
  "cssClass?": "string"
}
```

```json DataTableColumn
{
  "key": "string (required)",
  "header": "string (required)",
  "sortable?": false,
  "format?": "string",
  "width?": "string",
  "minWidth?": "string",
  "maxWidth?": "string",
  "headerClass?": "string",
  "cellClass?": "string"
}
```

For the complete protocol schema, see [DataTable](/protocol/data-table), [DataTableColumn](/protocol/data-table-column).
