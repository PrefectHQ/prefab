...
with Dialog(title="Delete Item", description="This action cannot be undone."):
    ...
    with Row(gap=2, css_class="justify-end"):
        ...
        Button(
            ...
            variant="destructive",
            on_click=[
                ToolCall("delete_item", arguments={"id": "{{ item_id }}"}),
                CloseOverlay(),
            ],
        )
```
...
Wire `ShowToast` callbacks onto a `ToolCall` to give feedback after the server responds. `CloseOverlay()` in the `on_success` list dismisses the dialog once the operation succeeds; `on_error` keeps the dialog open so the user can retry.

```python
from prefab_ui.components import Button, Dialog, Row, Text
from prefab_ui.actions import CloseOverlay, ShowToast, ToolCall
