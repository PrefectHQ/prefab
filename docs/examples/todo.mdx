...
with Row(gap=6, align="start", css_class="flex-wrap"):
    ...
    with ForEach("groups", let=dict(gi="{{ $index }}", show_done="{{ $item.show_done }}")):
        with Card(css_class="w-64"):
            ...
            with CardContent():
                with Column(gap=4):
                    # New todo input + add button (disabled until you type something)
                    with Row(gap=2):
    ...
    with Card(css_class="w-64 border-dashed p-3"):
        Button(
            "+",
            variant="ghost",
            size="lg",
...
{
  "view": {
    "cssClass": "gap-6 items-start flex-wrap",
    "type": "Row",
    "children": [
      {
        "let": {
      ...
      {
        "cssClass": "w-64 border-dashed p-3",
        "type": "Card",
        "children": [
          {
