...
<ComponentPreview json={{"view": {"cssClass": "gap-1", "type": "Column", "children": [{"type": "ForEach", "key": "steps", "children": [{"type": "Text", "content": "{{ $index + 1 }}. {{ $item }}"}]}]}, "state": {"steps": ["Collect underpants", "?", "Profit"]}}}>
<CodeGroup>
```python Python icon="python"
