---
title: DataTable
description: High-level data table with sorting, filtering, and pagination.
icon: table-cells
---

import { ComponentPreview } from '/snippets/component-preview.mdx'

DataTable takes flat `columns` and `rows` and renders a full-featured table with optional sorting, search filtering, and pagination. Use it when you have structured data and want interactive table features without building the markup yourself.

## Basic Usage

<ComponentPreview auto json={`{"view":{"type":"DataTable","columns":[{"key":"name","header":"Name","sortable":true},{"key":"email","header":"Email","sortable":false},{"key":"role","header":"Role","sortable":true}],"rows":[{"name":"Alice Johnson","email":"alice@example.com","role":"Admin"},{"name":"Bob Smith","email":"bob@example.com","role":"Editor"},{"name":"Carol White","email":"carol@example.com","role":"Viewer"}],"searchable":true,"paginated":false,"pageSize":10}}`}>
<CodeGroup>
```python Python icon="python"
from prefab_ui.components import DataTable, DataTableColumn

DataTable(
    columns=[
        DataTableColumn(key="name", header="Name", sortable=True),
        DataTableColumn(key="email", header="Email"),
        DataTableColumn(key="role", header="Role", sortable=True),
    ],
    rows=[
        {"name": "Alice Johnson", "email": "alice@example.com", "role": "Admin"},
        {"name": "Bob Smith", "email": "bob@example.com", "role": "Editor"},
        {"name": "Carol White", "email": "carol@example.com", "role": "Viewer"},
    ],
    searchable=True,
)
```
```json Protocol icon="brackets-curly"
{
  "view": {
    "type": "DataTable",
    "columns": [
      {
        "key": "name",
        "header": "Name",
        "sortable": true
      },
      {
        "key": "email",
        "header": "Email",
        "sortable": false
      },
      {
        "key": "role",
        "header": "Role",
        "sortable": true
      }
    ],
    "rows": [
      {
        "name": "Alice Johnson",
        "email": "alice@example.com",
        "role": "Admin"
      },
      {
        "name": "Bob Smith",
        "email": "bob@example.com",
        "role": "Editor"
      },
      {
        "name": "Carol White",
        "email": "carol@example.com",
        "role": "Viewer"
      }
    ],
    "searchable": true,
    "paginated": false,
    "pageSize": 10
  }
}
```
</CodeGroup>
</ComponentPreview>

## Pagination

Enable pagination to handle large datasets. The table shows page controls and displays `page_size` rows at a time.

<ComponentPreview auto json={`{"view":{"type":"DataTable","columns":[{"key":"id","header":"ID","sortable":false},{"key":"task","header":"Task","sortable":true},{"key":"status","header":"Status","sortable":false}],"rows":[{"id":"T-001","task":"Design system","status":"Complete"},{"id":"T-002","task":"API integration","status":"In Progress"},{"id":"T-003","task":"Testing","status":"Pending"},{"id":"T-004","task":"Documentation","status":"In Progress"},{"id":"T-005","task":"Deployment","status":"Pending"}],"searchable":false,"paginated":true,"pageSize":3,"caption":"Sprint tasks"}}`}>
<CodeGroup>
```python Python icon="python"
from prefab_ui.components import DataTable, DataTableColumn

DataTable(
    columns=[
        DataTableColumn(key="id", header="ID"),
        DataTableColumn(key="task", header="Task", sortable=True),
        DataTableColumn(key="status", header="Status"),
    ],
    rows=[
        {"id": "T-001", "task": "Design system", "status": "Complete"},
        {"id": "T-002", "task": "API integration", "status": "In Progress"},
        {"id": "T-003", "task": "Testing", "status": "Pending"},
        {"id": "T-004", "task": "Documentation", "status": "In Progress"},
        {"id": "T-005", "task": "Deployment", "status": "Pending"},
    ],
    paginated=True,
    page_size=3,
    caption="Sprint tasks",
)
```
```json Protocol icon="brackets-curly"
{
  "view": {
    "type": "DataTable",
    "columns": [
      {
        "key": "id",
        "header": "ID",
        "sortable": false
      },
      {
        "key": "task",
        "header": "Task",
        "sortable": true
      },
      {
        "key": "status",
        "header": "Status",
        "sortable": false
      }
    ],
    "rows": [
      {
        "id": "T-001",
        "task": "Design system",
        "status": "Complete"
      },
      {
        "id": "T-002",
        "task": "API integration",
        "status": "In Progress"
      },
      {
        "id": "T-003",
        "task": "Testing",
        "status": "Pending"
      },
      {
        "id": "T-004",
        "task": "Documentation",
        "status": "In Progress"
      },
      {
        "id": "T-005",
        "task": "Deployment",
        "status": "Pending"
      }
    ],
    "searchable": false,
    "paginated": true,
    "pageSize": 3,
    "caption": "Sprint tasks"
  }
}
```
</CodeGroup>
</ComponentPreview>

## API Reference

<Card icon="code" title="DataTable Parameters">
<ParamField body="columns" type="list[DataTableColumn]" required>
  Column definitions. Each column specifies a `key`, `header`, and optional `sortable` flag.
</ParamField>

<ParamField body="rows" type="list[dict] | str" default="[]">
  Row data as a list of dicts, or a `{{ field }}` interpolation reference.
</ParamField>

<ParamField body="searchable" type="bool" default="False">
  Show a search/filter input above the table.
</ParamField>

<ParamField body="paginated" type="bool" default="False">
  Show pagination controls below the table.
</ParamField>

<ParamField body="page_size" type="int" default="10">
  Number of rows per page when `paginated=True`.
</ParamField>

<ParamField body="caption" type="str | None" default="None">
  Optional table caption.
</ParamField>

<ParamField body="css_class" type="str | None" default="None">
  Additional Tailwind CSS classes.
</ParamField>
</Card>

<Card icon="code" title="DataTableColumn Parameters">
<ParamField body="key" type="str" required>
  Data key — matches keys in the row dicts.
</ParamField>

<ParamField body="header" type="str" required>
  Column header display text.
</ParamField>

<ParamField body="sortable" type="bool" default="False">
  Enable click-to-sort on this column.
</ParamField>
</Card>

## Protocol Reference

<Card icon="braces" title="DataTable Protocol">
<ParamField body="cssClass" type="string | null">
  CSS/Tailwind classes for styling
</ParamField>

<ParamField body="type" type="DataTable" default="DataTable">
  Component discriminator.
</ParamField>

<ParamField body="columns" type="array" required>
  Column definitions
</ParamField>

<ParamField body="rows" type="Action[] | string">
  Row data or `{{ interpolation }}` reference
</ParamField>

<ParamField body="searchable" type="boolean" default="false">
  Show search/filter input
</ParamField>

<ParamField body="paginated" type="boolean" default="false">
  Show pagination controls
</ParamField>

<ParamField body="pageSize" type="number" default="10">
  Rows per page when paginated
</ParamField>

<ParamField body="caption" type="string | null">
  Optional table caption
</ParamField>

</Card>

<Card icon="braces" title="DataTableColumn Protocol">
<ParamField body="key" type="string" required>
  Data key to display in this column
</ParamField>

<ParamField body="header" type="string" required>
  Column header text
</ParamField>

<ParamField body="sortable" type="boolean" default="false">
  Enable sorting for this column
</ParamField>

</Card>
