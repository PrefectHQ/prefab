...
<Info>
  All file reading happens in the browser. Files are never sent to a server unless you explicitly wire up an `on_success` action (like `ToolCall`) to do so.
</Info>
...
Files are read client-side, but they don't go anywhere until you tell them to. The typical pattern is to nest a `ToolCall` inside `on_success` — pick the file, then send its base64 data to your MCP server for processing.
...
Button(
    ...
    on_click=OpenFilePicker(
        max_size=5_000_000,
        on_success=ToolCall("upload", arguments={"file": "{{ $event }}"}),
    ),
)
```
...
The `on_error` callback on the nested `ToolCall` fires if the server-side processing fails. The error message is available as `$error`.

```python With error handling
from prefab_ui.components import Button
from prefab_ui.actions import OpenFilePicker, ToolCall, ShowToast
