...
To let an agent perform that operation on the open app too, define an [app-provided tool](/mcp/overview#app-provided-tools). It combines the backend call, app-specific arguments, response actions, and a result for the agent. A button can invoke the same definition with `.invoke()`.
...
<ParamField body="on_success" type="Action | list[Action] | None" default="None">
  Action(s) to run when the tool call succeeds. The returned value is available as `$result`.
</ParamField>

<ParamField body="on_error" type="Action | list[Action] | None" default="None">
  Action(s) to run when the tool call fails. The error message is available as `$error`.
</ParamField>
</Card>
