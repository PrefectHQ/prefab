...
For real-time charts that update from a server, use `on_mount` with `SetInterval` to poll for new data. Set `animate=False` to prevent distracting interpolation between updates:
...
mcp = FastMCP("Live Chart")
points = []

@mcp.tool(app=True)
def monitor() -> PrefabApp:
    with PrefabApp(
        state={"points": []},
        on_mount=SetInterval(
            ...
            on_tick=CallTool(
                "get_data",
                on_success=SetState("points", RESULT),
            ),
        ),
    ) as app:
        with Column(gap=4, css_class="p-6"):
            ...
            LineChart(
                data="{{ points }}",
                series=[ChartSeries(data_key="value", label="CPU %")],
                x_axis="time",
                height=300,
                animate=False,
            )
...
def get_data() -> list[dict]:
    points.append({"time": len(points), "value": random.randint(20, 80)})
    return points[-30:]  # keep last 30 points
```
