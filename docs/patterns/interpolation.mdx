...
Every string prop on every component supports `{{ }}` template interpolation. When the renderer encounters `{{ key }}` in a string value, it replaces it with the current value of `key` from [state](/components/state). No opt-in required — it works everywhere.
...
1. **Local scope** — values provided by [State](/components/state) or [ForEach](/components/foreach) components
2. **Global state** — values from `AppResult(state={...})`, named form controls, or [SetState](/actions/set-state) actions
...
The [State](/components/state) component provides local interpolation values to a subtree. Inner values shadow outer ones, and children see the merged context:
...
See the [State](/components/state) and [ForEach](/components/foreach) docs for more on scoping and iteration.
