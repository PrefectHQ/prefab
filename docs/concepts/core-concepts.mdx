...
The [Actions](/concepts/actions) guide covers action targets, chaining, the `$event` variable, and common patterns like loading state and optimistic updates.
...
Your MCP tools can return component trees that become live interfaces inside a supporting host. The renderer runs inside the host; your Python server handles backend tool calls. The same components, state, and actions drive the UI. The [MCP Apps](/mcp/overview) guide explains how to connect it to the agent and conversation; [FastMCP](/running/fastmcp) covers registering and serving it.
