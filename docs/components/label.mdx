...
<ComponentPreview auto json={`{"$protocol":"prefab","view":{"type":"Label","text":"Email address"}}`}>
<CodeGroup>
...
<ComponentPreview auto json={`{"$protocol":"prefab","view":{"cssClass":"gap-2","type":"Column","children":[{"type":"Label","text":"Username","forId":"username"},{"type":"Input","inputType":"text","placeholder":"Enter username","name":"username","disabled":false,"required":false}]}}`}>
<CodeGroup>
```python Python icon="python"
...
For the complete protocol schema, see the [Label protocol definition](/protocol/label).
