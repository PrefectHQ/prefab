...
state = set_initial_state(
    a=1, b=1, c=1, d=1, e=1,
    total=Computed("{{ a + b + c + d + e }}"),
)
```

`state.total` returns an `Rx("total")` reference, just like any other state key. In the rendered app, `{{ total }}` resolves to the current sum of all five values and updates automatically whenever any of them changes.

This is useful when the same derived value feeds into multiple components. Instead of duplicating `{{ a + b + c + d + e }}` across text labels, conditions, and action thresholds, you define it once and reference the result by name:

```python
# Five sliders, each showing its normalized percentage
Slider(name="a", min=0, max=100)
Text(f"{state.a} / {state.total} = {(state.a / state.total * 100).round(1)}%")
```
