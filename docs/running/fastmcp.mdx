...
2. **PrefabApp → structuredContent** — FastMCP serializes your return value into the Prefab JSON envelope, including the view, initial state, and any app tools
3. **Renderer** — the host loads the registered Prefab renderer resource and passes it the structured content. The renderer builds the UI.

When the user interacts with the UI (clicks a button, submits a form), [`CallTool`](/actions/call-tool) can send a request through MCP back to your server. Its response can contain data or a new component tree; response actions decide how to apply it to the UI. [App-provided tools](/mcp/overview#app-provided-tools) let the agent invoke an interaction on the open app as well.
...
Once your app is running, its [MCP host](/mcp/overview) connects it to the
conversation. Use [Messages](/actions/send-message) for user-initiated follow-up
requests and [Model Context](/actions/update-context) to share the current
selection or other useful state. [App-provided tools](/mcp/overview#app-provided-tools) let the agent invoke
operations on the open view, and [Display Modes](/actions/request-display-mode)
let the UI request more space. Availability depends on the host.
