...
State keys can contain dots to reach into nested structures. An integer segment means an array index, so `todos.0.done` means "the `done` field of the first item in the `todos` array."
