...
| `round` | decimal places (default 0) | `{{ 3.14159 \| round:2 }}` | `3.14` |
| `abs` | | `{{ -42 \| abs }}` | `42` |
...
`pluralize` is especially useful after `length` to label dynamic counts: `{{ items | length }} {{ items | length | pluralize:'item' }}` produces "1 item" or "3 items".
    ...
    "children": [
      {
        ...
        "type": "Row",
        "children": [
          {"type": "Badge", "label": "{{ skills | first }}", "variant": "default"},
          {"type": "Badge", "label": "{{ skills | last }}", "variant": "outline"}
        ]
      },
      {"content": "{{ skills | join }}", "type": "Text"},
      {"content": "{{ skills | length }} skills listed", "type": "Text"}
    ]
  },
  "state": {"skills": ["Python", "TypeScript", "Rust", "Go"]}
}
```
</CodeGroup>
</ComponentPreview>
