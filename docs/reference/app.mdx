...
The natural way to build an app is as a context manager. Components inside the `with` block become the view:
...
**`PrefabApp.from_json(wire, **overrides)`** wraps pre-serialized wire protocol data (e.g. from sandboxed execution). Keyword arguments override values from the wire:
