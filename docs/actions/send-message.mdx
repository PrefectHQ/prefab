...
`SendMessage` lets a UI control continue the conversation on the user's behalf. A button can ask the agent to explain a chart, compare selected items, or help with the next step. Its message can include the app's current selection, so the user can ask about what they see without typing those details again.
...
The renderer sends the message with the user role. The MCP host controls how it appears in the conversation and whether it starts an agent response. To share a selection for a later turn, use [Model Context](/actions/update-context).
