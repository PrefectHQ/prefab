...
Three cards with different data, but the card structure is defined once. The kwargs passed to `Use` become [interpolation](/expressions/templates) values scoped to that instance of the template.
