...
The most common pattern: use `ShowToast` as `on_success` and `on_error` callbacks on a `ToolCall` to give feedback after a server action completes.
...
Wire these onto a `ToolCall` with `on_success` and `on_error`. The `{{ $error }}` variable captures the error message from the server.
