...
The first argument to `If` and `Elif` is a [reactive expression](/concepts/expressions). With `Rx`, you can use Python operators directly:
...
Any expression the language supports — comparisons, boolean logic, [pipes](/concepts/expressions#formatting-with-pipes) — works as a condition.
