...
Three cards with different data, but the card structure is defined once. The kwargs passed to `Use` become [interpolation](/concepts/expressions) values scoped to that instance of the template.
