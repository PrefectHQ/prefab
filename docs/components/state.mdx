...
<ComponentPreview auto json={`{"view":{"type":"State","state":{"prefix":"Speaks"},"children":[{"cssClass":"gap-2 items-center","type":"Row","children":[{"type":"ForEach","key":"languages","children":[{"type":"Badge","label":"{{ prefix }}: {{ $item }}","variant":"secondary"}]}]}]},"languages":["Python","TypeScript","Rust"]}`}>
<CodeGroup>
```python Python icon="python"
