...
```python
from prefab_ui.components import Button, Slider
from prefab_ui import ToolCall, SetState

Button("Run Analysis", on_click=ToolCall("analyze"))
Slider(name="threshold", on_change=SetState("threshold"))
```
...
| Action | Purpose |
|--------|---------|
| [State](/apps/actions/set-state) | Set or toggle state variables (captures event values by default) |
| [Open Link](/apps/actions/open-link) | Open a URL through the host's link handler |
...
| Action | Purpose |
|--------|---------|
| [Call Tool](/apps/actions/call-tool) | Call a server-side tool with optional arguments |
| [Send Message](/apps/actions/send-message) | Send a message to the chat as if the user typed it |
| [Update Context](/apps/actions/update-context) | Silently update what the model knows |
...
This works for all named controls: `Input`, `Textarea`, `Select`, `Slider`, `Checkbox`, `Switch`, and `RadioGroup`. The `name` is the state key.
