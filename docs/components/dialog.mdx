...
Combine a confirmation Dialog with `ShowToast` callbacks to give feedback after the server responds. The `on_success` and `on_error` callbacks on `ToolCall` fire after the tool completes.

```python Confirmation with Toast Feedback
