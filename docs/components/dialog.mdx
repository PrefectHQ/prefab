---
title: Dialog
description: Modal overlay with a trigger and content panel.
icon: window-maximize
---

import { ComponentPreview } from '/snippets/component-preview.mdx'

Dialogs display content in a modal overlay that focuses user attention. Like Popover, the first child becomes the trigger and the remaining children form the dialog body. An optional `title` and `description` render in the dialog header.

## Basic Usage

<ComponentPreview auto height="400px" json={`{"view":{"type":"Dialog","title":"Crew Profile","description":"Update your details for the Heart of Gold crew manifest.","children":[{"type":"Button","label":"Edit Profile","variant":"outline","size":"default","disabled":false},{"cssClass":"gap-3","type":"Column","children":[{"type":"Label","text":"Name"},{"type":"Input","inputType":"text","placeholder":"Arthur Dent","name":"crewName","disabled":false,"required":false},{"type":"Label","text":"Designation"},{"type":"Input","inputType":"text","placeholder":"Sandwich Maker","name":"crewDesignation","disabled":false,"required":false}]},{"cssClass":"gap-2 justify-end","type":"Row","children":[{"type":"Button","label":"Save changes","variant":"default","size":"default","disabled":false}]}]}}`}>
<CodeGroup>
```python Python icon="python"
from prefab_ui.components import (
    Button,
    Column,
    Dialog,
    Input,
    Label,
    Row,
)

with Dialog(
    title="Crew Profile",
    description="Update your details for the Heart of Gold crew manifest.",
):
    Button("Edit Profile", variant="outline")
    with Column(gap=3):
        Label("Name")
        Input(name="crewName", placeholder="Arthur Dent")
        Label("Designation")
        Input(name="crewDesignation", placeholder="Sandwich Maker")
    with Row(gap=2, css_class="justify-end"):
        Button("Save changes")
```
```json Protocol icon="brackets-curly"
{
  "view": {
    "type": "Dialog",
    "title": "Crew Profile",
    "description": "Update your details for the Heart of Gold crew manifest.",
    "children": [
      {
        "type": "Button",
        "label": "Edit Profile",
        "variant": "outline",
        "size": "default",
        "disabled": false
      },
      {
        "cssClass": "gap-3",
        "type": "Column",
        "children": [
          {
            "type": "Label",
            "text": "Name"
          },
          {
            "type": "Input",
            "inputType": "text",
            "placeholder": "Arthur Dent",
            "name": "crewName",
            "disabled": false,
            "required": false
          },
          {
            "type": "Label",
            "text": "Designation"
          },
          {
            "type": "Input",
            "inputType": "text",
            "placeholder": "Sandwich Maker",
            "name": "crewDesignation",
            "disabled": false,
            "required": false
          }
        ]
      },
      {
        "cssClass": "gap-2 justify-end",
        "type": "Row",
        "children": [
          {
            "type": "Button",
            "label": "Save changes",
            "variant": "default",
            "size": "default",
            "disabled": false
          }
        ]
      }
    ]
  }
}
```
</CodeGroup>
</ComponentPreview>

## Confirmation Pattern

Dialogs work well for destructive actions that need explicit confirmation before proceeding.

<ComponentPreview auto height="360px" json={`{"view":{"type":"Dialog","title":"Activate Improbability Drive","description":"This action cannot be undone.","children":[{"type":"Button","label":"Engage Drive","variant":"destructive","size":"default","disabled":false},{"type":"Text","content":"Are you sure? The last time this was engaged, a sperm whale and a bowl of petunias materialized in mid-air."},{"cssClass":"gap-2 justify-end","type":"Row","children":[{"type":"Button","label":"Cancel","variant":"outline","size":"default","disabled":false},{"type":"Button","label":"Engage","variant":"destructive","size":"default","disabled":false}]}]}}`}>
<CodeGroup>
```python Python icon="python"
from prefab_ui.components import Button, Dialog, Row, Text

with Dialog(
    title="Activate Improbability Drive",
    description="This action cannot be undone.",
):
    Button("Engage Drive", variant="destructive")
    Text(
        "Are you sure? The last time this was engaged, "
        "a sperm whale and a bowl of petunias materialized in mid-air."
    )
    with Row(gap=2, css_class="justify-end"):
        Button("Cancel", variant="outline")
        Button("Engage", variant="destructive")
```
```json Protocol icon="brackets-curly"
{
  "view": {
    "type": "Dialog",
    "title": "Activate Improbability Drive",
    "description": "This action cannot be undone.",
    "children": [
      {
        "type": "Button",
        "label": "Engage Drive",
        "variant": "destructive",
        "size": "default",
        "disabled": false
      },
      {
        "type": "Text",
        "content": "Are you sure? The last time this was engaged, a sperm whale and a bowl of petunias materialized in mid-air."
      },
      {
        "cssClass": "gap-2 justify-end",
        "type": "Row",
        "children": [
          {
            "type": "Button",
            "label": "Cancel",
            "variant": "outline",
            "size": "default",
            "disabled": false
          },
          {
            "type": "Button",
            "label": "Engage",
            "variant": "destructive",
            "size": "default",
            "disabled": false
          }
        ]
      }
    ]
  }
}
```
</CodeGroup>
</ComponentPreview>

## API Reference

<Card icon="code" title="Dialog Parameters">
<ParamField body="title" type="str | None" default="None">
  Dialog header title.
</ParamField>

<ParamField body="description" type="str | None" default="None">
  Dialog header description text.
</ParamField>

<ParamField body="css_class" type="str | None" default="None">
  Additional Tailwind CSS classes.
</ParamField>
</Card>

## Protocol Reference

<Card icon="braces" title="Dialog Protocol">
<ParamField body="cssClass" type="string | null">
  CSS/Tailwind classes for styling
</ParamField>

<ParamField body="children" type="array">
</ParamField>

<ParamField body="type" type="Dialog" default="Dialog">
  Component discriminator.
</ParamField>

<ParamField body="title" type="string | null">
  Dialog header title
</ParamField>

<ParamField body="description" type="string | null">
  Dialog header description
</ParamField>

</Card>
