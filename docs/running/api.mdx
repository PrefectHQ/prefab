...
description: Wire Prefab apps to a REST backend with FastAPI.
icon: server
---
...
from prefab_ui.components import Column, ForEach, Input, Text
...
Pass query parameters with `params` — values support `{{ key }}` interpolation:
...
Reference named inputs with `{{ key }}` — make sure each key is initialized in your `PrefabApp(state=...)`, otherwise unset values render as literal template strings.
...
The [`examples/fastapi`](https://github.com/PrefectHQ/prefab/tree/main/examples/fastapi) directory contains a complete working app — a Hitchhiker's Guide catalog with live search, dialog-based entry creation, inline deletion, and error handling.
