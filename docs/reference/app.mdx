...
| `app_tools` | `list[AppTool]` | [App-provided tools](/mcp/overview#app-provided-tools) an MCP host can expose to the agent for this running app |
...
| `stylesheets` | `list[str]` | External CSS URLs to load as `<link>` tags |
| `css` | `list[str]` | Inline CSS strings to load as `<style>` tags |
| `mode` | `"light" \| "dark"` | Force light or dark mode |
...
**`app.to_json()`** returns the wire-format envelope — a dict with `$prefab`, `view`, `state`, `tools`, `defs`, `css`, `stylesheets`, and `mode` as needed.
...
Use `stylesheets` for external CSS URLs. They render as `<link>` tags and contribute style origins to `app.csp()`.
