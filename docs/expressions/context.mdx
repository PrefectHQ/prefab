...
The rule of thumb: use `set_initial_state()` for anything the user can change (form inputs, toggles, lists that get edited). Use `let` for fixed data you're passing into a section: labels, configuration, static values that don't change at runtime. `let` bindings are read-only; user interactions cannot modify them.
