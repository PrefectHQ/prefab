...
sidebarTitle: Reactive Expressions
...
When authoring in Python, use the `.rx` property on stateful components to generate these expressions automatically rather than writing `{{ }}` by hand. `input.rx` in an f-string becomes `{{ input-1 }}` in the protocol. Write `{{ }}` directly for runtime variables like `$item` or for expressions that combine multiple state keys.

```python
from prefab_ui.components import Badge, Input, Text

amount = Input()
Text(f"Total: {amount.rx}")                    # uses .rx — cleaner Python
Text("{{ price * quantity | currency }}")      # expression combining two keys
