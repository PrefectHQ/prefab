...
Use the `.rx` property on a stateful component to pass its current value to the tool. Assign the component to a variable, then reference `.rx` in the arguments dict — it serializes to `{{ key }}` at build time and re-evaluates at call time.

<ComponentPreview json={{"view":{"cssClass":"gap-3","type":"Column","children":[{"type":"Input","inputType":"text","placeholder":"Enter a city...","name":"input-1","disabled":false,"required":false},{"type":"Button","label":"Get Weather","variant":"default","size":"default","disabled":false,"onClick":{"action":"toolCall","tool":"get_weather","arguments":{"location":"{{ input-1 }}"}}}]}}}>
<CodeGroup>
```python Python icon="python"
...
For the complete protocol schema, see [CallTool](/protocol/call-tool).
