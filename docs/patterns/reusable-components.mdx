...
`Define` serializes to the template body itself. Its name becomes a key in the `defs` envelope:
...
With overrides, Use adds `let` bindings to the `$ref` node that scope the interpolation values:
