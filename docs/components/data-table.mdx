...
Enable `selectable` to add checkboxes for multi-row selection. Rows accumulate selection state locally — no action fires per selection. Use this for bulk operations where you want the user to pick a set of rows before acting.
...
Use `on_row_click` to make rows clickable. Clicking any row fires the action with the row's data dict available as `$event`. Use this for navigation or detail views where each row leads somewhere.
...
DataTable(
    ...
    on_row_click=ShowToast("Selected: {{ $event.name }}"),
)
```
</ComponentPreview>
