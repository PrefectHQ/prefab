...
<ComponentPreview json={{"view":{"cssClass":"gap-3 items-center","type":"Row","children":[{"type":"Button","label":"-","variant":"outline","size":"default","disabled":false,"onClick":{"action":"setState","key":"count","value":"{{ count - 1 }}"}},{"content":"{{ count }}","type":"Text"},{"type":"Button","label":"+","variant":"outline","size":"default","disabled":false,"onClick":{"action":"setState","key":"count","value":"{{ count + 1 }}"}}]},"state":{"count":0}}} playground="H4sIAL5FuGkC_6WOzW7CMAzH73kKK6dUFATiNqmXPQLsXnlRCtaKHaXuQEJ996VFBQHH-WT5__Fzk-QEMYUGv-ueVhgj0ClKUuiC1sSkhG3dKWowzbPXSzZyYO3myGevKlzCTs4lfIWLvkbQKwnf_fug-6nZvMGcl561WhfGnEmPY6U7YKy2JWBLB66sz-SQbPFhIM8N7ezSlvCLiTCHrfTaEod8Eq59S_6nmpHOToAs2esVph2WsIFhsEUxNY7_u4c4Ck-kxT9IizvpD3GawUiAAQAA">
<CodeGroup>
...
<ComponentPreview json={{"view":{"cssClass":"gap-3","type":"Column","children":[{"name":"name","type":"Input","inputType":"text","placeholder":"Your name","disabled":false,"readOnly":false,"required":false},{"content":"Hello, {{ name }}!","type":"Text"}]},"state":{"name":"Arthur"}}} playground="H4sIAL5FuGkC_2WNuwrCQBBF-_2KcasEFhs7IYXYaG9jFcY4MQv7GDazKIT8u8mihTjFbc65d_oUPXCiHm9ttltkBus5JoGRpLXBikXXjoJCqv91u7iIgYKM38oxuuyDgXPgLAYu9BKl_naqgJ4afUgy5KRrpZ5Whk-3eiA3u3qvYLky87HX1AbYYUdDdHdKjb7GnKCAuvjrv0qfyLloYJoKgnneLPgNdsPwa-cAAAA">
<CodeGroup>
```python Python icon="python"
from prefab_ui.app import set_initial_state
...
<ComponentPreview json={{"view":{"cssClass":"gap-3","type":"Column","children":[{"name":"slider_23","type":"Slider","min":0.0,"max":100.0,"value":50.0,"disabled":false},{"content":"Volume: {{ slider_23 }}%","type":"Text"},{"type":"Progress","value":"{{ slider_23 }}","min":0,"max":100,"variant":"default"}]},"state":{"slider_23":50.0}}} playground="H4sIAL5FuGkC_z3NTQrCMBAF4H1OMRSEBEKJiJtCVl6goLiVqJMaaJIySbUg3t3-6WxmFt97Yyl66AituV56V96i72LAkBO48aIMh9j2PkioKTaEKUk4tu6OJOGEQ2bs5fJjRbwxnd6JisE4aVagV86fpu1R75UE74Kelhn0Vikx66mL2-I89WAF7yVd0vDZFIv4_V-L_kCwLwwpKzDDAAAA">
<CodeGroup>
```python Python icon="python"
...
```json Protocol icon="brackets-curly"
{
  "view": {
    ...
    "children": [
      ...
      {"content": "Volume: {{ slider_23 }}%", "type": "Text"},
      {
        "type": "Progress",
        "value": "{{ slider_23 }}",
        "min": 0,
...
Python closures capture the *name* `slider`, not its current value, so the lambda evaluates successfully once `slider` exists. See [Forward References](/concepts/composition#forward-references) for more.

Use `.rx` when you have a component variable in scope. For keys from `set_initial_state()`, use the returned proxy — `state = set_initial_state(count=0)` gives you `state.count`, which is equivalent to `Rx("count")`. Use `Rx("key")` directly when the state key was created by a form control's `name` prop elsewhere in the tree and you don't have the component variable handy.
