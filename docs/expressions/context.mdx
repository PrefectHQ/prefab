...
**Form controls** — any component with a `name` prop (or the auto-generated one) syncs its current value to state automatically. Assign the component to a variable and use `.rx` to reference that state key from other components and actions.
...
<ComponentPreview json={{"view":{"cssClass":"gap-3","type":"Column","children":[{"type":"Slider","min":0.0,"max":30.0,"step":1.0,"name":"inventory","disabled":false},{"content":"Inventory: {{ inventory }}","type":"Text"},{"type":"Condition","cases":[{"when":"{{ inventory == 0 }}","children":[{"type":"Alert","variant":"destructive","children":[{"type":"AlertDescription","content":"Out of stock!"}]}]},{"when":"{{ inventory > 0 && inventory < 10 }}","children":[{"type":"Alert","variant":"default","children":[{"type":"AlertDescription","content":"Low stock warning"}]}]}]}]},"state":{"inventory":15}}}>
<CodeGroup>
```python Python icon="python"
...
with Column(gap=3):
    inventory_slider = Slider(
        name="inventory",
        min=0,
        max=30,
        step=1,
    )
    Text(f"Inventory: {inventory_slider.rx}")
    with If("{{ inventory == 0 }}"):
