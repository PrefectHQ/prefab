...
State is deliberately simple: a flat map with dot-path addressing for nesting. Derived display values belong in [expressions](/concepts/expressions), where they're computed at render time from whatever state holds. For side effects that should run when a key changes, use watchers.
