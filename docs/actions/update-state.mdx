...
<ComponentPreview json={{"view":{"cssClass":"gap-3","type":"Column","children":[{"type":"Slider","min":0.0,"max":100.0,"name":"slider-1","disabled":false},{"content":"Brightness: {{ slider-1 }}%","type":"Muted"}]},"state":{"brightness":50}}}>
<CodeGroup>
...
with Column(gap=3):
    brightness_slider = Slider(min=0, max=100)
    Muted(f"Brightness: {brightness_slider.rx}%")
```
...
Drag the slider — the text updates instantly. The slider auto-generates a state key, and `brightness_slider.rx` gives the `Muted` component a reactive reference to it. Most form controls (Input, Checkbox, Switch, Select, etc.) work the same way — assign to a variable and use `.rx` to reference the current value anywhere in your UI.
...
with Column(gap=3):
    ...
    with Row(gap=2):
        new_member_input = Input(name="new_member", placeholder="Crew member name...")
        Button("Add", on_click=[
            AppendState("crew", new_member_input.rx),
            SetState("new_member", ""),
        ])
