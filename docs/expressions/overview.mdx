...
<ComponentPreview auto json={`{"view":{"cssClass":"gap-3 items-center","type":"Row","children":[{"type":"Button","label":"-","variant":"outline","size":"default","disabled":false,"onClick":{"action":"setState","key":"count","value":"{{ count - 1 }}"}},{"type":"Text","content":"{{ count }}"},{"type":"Button","label":"+","variant":"outline","size":"default","disabled":false,"onClick":{"action":"setState","key":"count","value":"{{ count + 1 }}"}}]},"count":0}`}>
<CodeGroup>
...
- **Conditional rendering** — `visible_when="inventory > 0"`
...
| **[ToolCall](/actions/call-tool)** | Run server logic | Data fetching, business logic, complex transformations |
...
**`visible_when`** controls whether a component renders at all. The expression evaluates to a truthy or falsy value, and the component only appears when the result is truthy.

```python
Alert("Low stock", visible_when="inventory > 0 && inventory < 10")
```

**ForEach `filter`** controls which items in a collection are shown. The expression evaluates once per item, with access to both the item's fields and the surrounding state.
