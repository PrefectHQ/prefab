...
The key idea: **state keys become template variables.** When you set `name="Arthur Dent"` in your state (via `set_data`, a `State` component, or a form input with a `name` attribute), the key `name` is available inside any `{{ }}` expression in that component's subtree. Write `{{ name }}` and it resolves to `"Arthur Dent"`.

## Basic Usage

In this example, the `State` component defines three keys — `name`, `role`, and `answer`. Each `{{ }}` template resolves its corresponding key from that state:

<ComponentPreview auto json={`{"view":{"type":"State","state":{"name":"Arthur Dent","role":"Reluctant Adventurer","answer":42},"children":[{"type":"Card","children":[{"type":"CardHeader","children":[{"type":"CardTitle","content":"{{ name }}"}]},{"type":"CardContent","children":[{"cssClass":"gap-1","type":"Column","children":[{"type":"Text","content":"Role: {{ role }}"},{"type":"Text","content":"The Answer to Everything: {{ answer }}"}]}]}]}]}}`}>
<CodeGroup>
...
with State(name="Arthur Dent", role="Reluctant Adventurer", answer=42):
    with Card():
        with CardHeader():
            CardTitle("{{ name }}")
        with CardContent():
...
<ComponentPreview auto json={`{"view":{"type":"State","state":{"name":"Zaphod Beeblebrox","role":"President of the Galaxy"},"children":[{"cssClass":"gap-2","type":"Column","children":[{"type":"Text","content":"{{ name }}"},{"type":"Badge","label":"{{ role }}","variant":"secondary"},{"type":"Input","inputType":"text","placeholder":"Message {{ name }}...","disabled":false,"required":false}]}]}}`}>
<CodeGroup>
```python Python icon="python"
from prefab_ui.components import (
    Badge,
...
<ComponentPreview auto json={`{"view":{"cssClass":"gap-3","type":"Column","children":[{"type":"Input","inputType":"text","placeholder":"First name","name":"first","disabled":false,"required":false},{"type":"Input","inputType":"text","placeholder":"Last name","name":"last","disabled":false,"required":false},{"type":"Text","content":"{{ first + ' ' + last }}"}]},"first":"Arthur","last":"Dent"}`}>
<CodeGroup>
```python Python icon="python"
