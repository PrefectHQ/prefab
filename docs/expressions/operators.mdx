...
<ComponentPreview auto json={`{"view":{"cssClass":"gap-3","type":"Column","children":[{"cssClass":"gap-2 items-center","type":"Row","children":[{"type":"Text","content":"Quantity:"},{"type":"Button","label":"-","variant":"outline","size":"default","disabled":false,"onClick":{"action":"setState","key":"quantity","value":"{{ quantity - 1 }}"}},{"type":"Text","content":"{{ quantity }}"},{"type":"Button","label":"+","variant":"outline","size":"default","disabled":false,"onClick":{"action":"setState","key":"quantity","value":"{{ quantity + 1 }}"}}]},{"type":"Text","content":"Total: {{ price * quantity | currency }}"}]},"price":42,"quantity":1}`}>
<CodeGroup>
...
```json Protocol icon="brackets-curly"
{
  "view": {
    ...
    "children": [
      ...
      {
        "type": "Text",
        "content": "Total: {{ price * quantity | currency }}"
      }
    ]
  }
}
...
<ComponentPreview auto json={`{"view":{"cssClass":"gap-3","type":"Column","children":[{"cssClass":"gap-2","type":"Row","children":[{"type":"Input","inputType":"text","placeholder":"First name","name":"first","disabled":false,"required":false},{"type":"Input","inputType":"text","placeholder":"Last name","name":"last","disabled":false,"required":false}]},{"type":"Text","content":"Hello, {{ first + ' ' + last }}!"}]},"first":"Arthur","last":"Dent"}`}>
<CodeGroup>
```python Python icon="python"
from prefab_ui.components import Column, Input, Row, Text

set_data(first="Arthur", last="Dent")
...
Comparisons are commonly used in `visible_when` conditions to show or hide elements based on state:

```python
Alert("Out of stock", visible_when="inventory == 0")
Alert("Low stock", visible_when="inventory > 0 && inventory < 10")
```
...
The ternary operator `condition ? ifTrue : ifFalse` selects between two values. It's how you produce conditional text without `visible_when`:

<ComponentPreview auto json={`{"view":{"cssClass":"gap-2","type":"Column","children":[{"cssClass":"gap-3 items-center","type":"Row","children":[{"type":"Switch","checked":false,"size":"default","name":"active","disabled":false,"required":false,"onChange":{"action":"toggleState","key":"active"}},{"type":"Text","content":"{{ active ? 'Online' : 'Offline' }}"}]}]},"active":true}`}>
<CodeGroup>
```python Python icon="python"
from prefab_ui.components import Column, Row, Switch, Text
from prefab_ui import ToggleState

set_data(active=True)

with Column(gap=2):
    with Row(gap=3, align="center"):
        Switch(name="active", on_change=ToggleState("active"))
