...
**Prefab is a UI framework for building rich, interactive interfaces in Python.** Create [MCP Apps](/running/fastmcp), data dashboards, interactive tools, and more with 100+ prebuilt components. A bundled React renderer turns everything into a self-contained application that runs standalone or against any backend.
...
This card has a live-updating heading and a text input bound to client-side state. Named form controls automatically sync their values, and a reactive expression system binds that state back into any component. Type in the input and watch the heading update in real time:
