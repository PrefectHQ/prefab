...
<ParamField body="gradient" type="bool | None" default="None">
  Gradient stroke. `None` inherits the theme default, `True` forces a gradient on, `False` forces a solid stroke.
</ParamField>
...
```json Ring
{
  ...
  "max?": 100,
  "label?": "string",
  "variant?": "default | success | warning | destructive | info | muted | string",
  "size?": "sm | default | lg",
  "thickness?": 6,
