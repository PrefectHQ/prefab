...
When the same key can change from multiple sources (different buttons, server responses, user inputs), attaching `on_change` to each component that writes it becomes tedious. `Watch` centralizes the reaction: it fires its `on_change` actions whenever the watched key changes, regardless of what caused the change. See [Watching for changes](/concepts/state#watching-for-changes) for details.
