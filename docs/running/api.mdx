...
Reference named inputs by assigning the component to a variable and using `.rx`, or with `{{ key }}` directly — make sure each key is initialized in your `PrefabApp(state=...)`, otherwise unset values render as literal template strings.
