...
with PrefabApp(state={"name": "world"}, css_class="max-w-md mx-auto") as app:
    with Card():
        with CardContent():
            with Column(gap=3):
...
Prefab components nest using Python's `with` statement — `Card`, `Column`, `Input` compose into a tree that compiles to JSON, which a bundled React renderer turns into real HTML. The [Composition](/concepts/components) page walks through this in detail.
