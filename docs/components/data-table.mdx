...
Add checkboxes for multi-row selection with `selectable=True`. The header checkbox toggles select-all. Selection is client-side state — no action fires per checkbox, so users can build up a selection before acting on it with a button or other control.
...
DataTable(
    columns=[
        DataTableColumn(key="name", header="Employee", sortable=True),
        DataTableColumn(key="department", header="Department"),
        DataTableColumn(key="status", header="Status"),
    ],
    rows=[
        {"name": "Alice Johnson", "department": "Engineering", "status": Badge("Active", variant="success")},
        {"name": "Bob Smith", "department": "Marketing", "status": Badge("Active", variant="success")},
        {"name": "Carol White", "department": "Engineering", "status": Badge("On Leave", variant="warning")},
        {"name": "Dan Lee", "department": "Sales", "status": Badge("Active", variant="success")},
    ],
    selectable=True,
    filter=True,
)
...
<ParamField body="selectable" type="bool" default="False">
  Show row selection checkboxes. Selection is visual state only — no action fires per selection.
</ParamField>
